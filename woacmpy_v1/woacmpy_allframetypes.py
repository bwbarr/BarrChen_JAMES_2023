...
def plot_3Dscat(fr,steplist):
    ...
    # Plot 3D scatter plot in which marker color shows value of third field.
    cmap = fr.typeparams[0]
    clims = fr.typeparams[1]
    cticks = fr.typeparams[2]
    if steplist is None:
        steplist = fr.runs[0].steplist
    for t in steplist:
        if fr.filter is None:
            xfield = fr.runs[0].myfields[fr.fldindx[0]][fr.doms[0]].grdata[t,:,:]
            yfield = fr.runs[0].myfields[fr.fldindx[1]][fr.doms[0]].grdata[t,:,:]
            zfield = fr.runs[0].myfields[fr.fldindx[2]][fr.doms[0]].grdata[t,:,:]
        else:
            xfield = fr.runs[0].myfields[fr.fldindx[0]][fr.doms[0]].grdata_filt[fr.filtindx[0][0]][t,:,:]
            yfield = fr.runs[0].myfields[fr.fldindx[1]][fr.doms[0]].grdata_filt[fr.filtindx[1][0]][t,:,:]
            zfield = fr.runs[0].myfields[fr.fldindx[2]][fr.doms[0]].grdata_filt[fr.filtindx[2][0]][t,:,:]
        plt.scatter(xfield.flatten(),yfield.flatten(),s=5,\
                  c=zfield.flatten(),cmap=cmap,vmin=clims[0],vmax=clims[1])
    colorbar = plt.colorbar(shrink=0.8,extend='both',ticks=cticks)
            ...
            for n in range(1,len(fr.fldindx)):
                ...
                if fr.typeparams[0] in ['mean','mean_1stdev']:
                    if fr.typeparams[2] == False:
                        if fr.typeparams[0] == 'mean' and fr.typeparams[5] == True \
                                and fr.fldname[0] == 'Rstorm':    # Integrate azimuthally
                            yvals = fr.runs[k].thing1[n-1][0]*xbin*1000*np.pi*2*1e-6    # Units change by factor of [m], value reduced by factor of 1e-6
                            plt.plot(xbin,yvals,                    fr.colors[k][n-1],linewidth=2,label=label,\
                                    linestyle=fr.typeparams[3][k])
                        else:
                            plt.plot(xbin,fr.runs[k].thing1[n-1][0],fr.colors[k][n-1],linewidth=2,label=label,\
                                    linestyle=fr.typeparams[3][k])
                    elif fr.typeparams[2]:
                        plt.plot(xbin,fr.runs[k].thing1[n-1][0],fr.colors[k][n-1],linewidth=2,label=label,\
                                linestyle=fr.typeparams[3][k],path_effects=[pe.Stroke(linewidth=4,foreground='w'),pe.Normal()])
                elif fr.typeparams[0] in ['med','med_quart']:
                    plt.plot(xbin,fr.runs[k].thing3[n-1][0],fr.colors[k][n-1],linewidth=2,label=label,\
                            linestyle=fr.typeparams[3][k])
        ...
        Type parameters (dictionary):
                    {
                     ...
                     '2Dfield':
                         [
                          ...
                          7: [[list of levels to draw contours],[list of (x,y) tuples for contour labels]], or [None] for nothing
                         ]
                     '1Dfield':
                         [
...
def plot_hovRstorm(fr,steplist_0):
    ...
    if fr.typeparams[1] in ['Diff1M0','PctD1V0']:    # Use run1 minus run0 difference or % diff of run1 versus run0
        ...
        if fr.filter is None:
            ...
            field0 = np.array([fr.runs[0].myfields[findx][fr.doms[0]].grdata_filt[fr.filtindx[1][0]][t,:,:] for t in steplist])
            field1 = np.array([fr.runs[1].myfields[findx][fr.doms[1]].grdata_filt[fr.filtindx[1][1]][t,:,:] for t in steplist])
        X = x0    # [km]
        Y = y0    # [km]
        field1_remapped = []
        ...
        if fr.filter is None:
            ...
            field2D = np.array([fr.runs[0].myfields[findx][fr.doms[0]].grdata_filt[fr.filtindx[1][0]][t,:,:] for t in steplist])
    # 2. Calculate azimuthal mean/total or 1D profile
    fieldHov = []
    if fr.typeparams[2] in ['Azimean','Azitot']:    # Calculate azimuthal mean or total
        if fr.typeparams[3] in ['FR','FL','RL','RR']:
        ...
        for t in range(len(steplist)):
            ...
            if fr.typeparams[2] == 'Azitot':    # Integrate azimuthally
        ...
        for n in range(1,len(fr.fldindx)):
            ...
            if fr.typeparams[1] == 'lines':
                ...
                if fr.typeparams[5][0]:    # Plot means, this only works for one pair of x,y fields because only one color can be given
                    ...
                    mec = 'w'     if Mfmt[2] else Mfmt[3]
                    plt.plot([np.mean(blkprops[0,:])],[np.mean(blkprops[n,:])],mec=mec,mfc=mfc,ls='',ms=Mfmt[1],marker=Mfmt[0],label=Mfmt[4],zorder=10,mew=1)
                    print('\n')
